...
class Wrapper:
    ...
    def setup_from_tag(self, load_bravais_lattice='all'):
        ...
        self.data_params['min_unit_cell'] = float(params['min_unit_cell'])
        self.data_params['max_unit_cell'] = float(params['max_unit_cell'])
        self.rf_params['unit_cell_length'] = self.data_params['unit_cell_length']
    ...
    def load_data(self):
        ...
        for index, bravais_lattice in enumerate(self.data_params['bravais_lattices']):
            file_name = os.path.join(
                ...
                os.path.join('generated_datasets', f'dataset_{bravais_lattice}.parquet')
                )
    ...
    def augment_data(self):
        ...
        self.augmentor = Augmentor(
            ...
            min_unit_cell=self.data_params['min_unit_cell'],
            max_unit_cell=self.data_params['max_unit_cell'],
    ...
    def plot_input(self):
        ...
        for split_group in self.data_params['split_groups']:
            ...
            axes[0].bar(centers, hist, width=dbin, label='Unaugmented')
            if self.data_params['augment']:
                ...
                axes[0].bar(centers, hist_augmented, width=dbin, alpha=0.5, label='Augmented')
            axes[0].legend()
            ...
            axes[1].bar(centers, hist, width=dbin)
            if self.data_params['augment']:
                ...
                axes[1].bar(centers, hist_augmented, width=dbin, alpha=0.5)
            ...
            for index in range(3):
                ...
                axes[index + 2].bar(centers, hist, width=dbin)
                if self.data_params['augment']:
                ...
                axes[index + 2].set_title(y_labels[index])
        ...
        for split_group in self.data_params['split_groups']:
            ...
            for index in range(6):
                ...
                axes[index].bar(centers, hist, width=dbin)
                if self.data_params['augment']:
                    hist_augmented, _ = np.histogram(
                        xnn_augmented[:, index], bins=bins, density=True
                        )
                    axes[index].bar(centers, hist_augmented, width=dbin, alpha=0.5)
                axes[index].set_title(xnn_labels[index])
    ...
    def setup_random(self):
        from mlindex.model_training.RandomGenerator import RandomGenerator
    ...
    def setup_random_forest(self):
        ...
        self.random_forest_generator = dict.fromkeys(self.data_params['split_groups'])
        for split_group_index, split_group in enumerate(self.data_params['split_groups']):
            self.random_forest_generator[split_group] = RandomForest(
                split_group,
                self.data_params,
                self.rf_params[split_group],
                self.save_to['random_forest'],
                self.random_seed,
                )
            self.random_forest_generator[split_group].setup()
            if self.rf_params[split_group]['load_from_tag']:
                self.random_forest_generator[split_group].load_from_tag()
            else:
                split_group_indices = self.data['split_group'] == split_group
                self.random_forest_generator[split_group].train(data=self.data[split_group_indices])
                self.random_forest_generator[split_group].load_from_tag()

    def inferences_random_forest(self):
        ...
        for split_group_index, split_group in enumerate(self.data_params['split_groups']):
            split_group_indices = self.data['split_group'] == split_group
            reindexed_uc_pred[split_group_indices, :], reindexed_uc_pred_var[split_group_indices, :], _ = \
                self.random_forest_generator[split_group].predict(data=self.data[split_group_indices])
    ...
    def evaluate_random_forest(self):
        ...
        for bravais_lattice in self.data_params['bravais_lattices']:
            evaluate_regression(
                ...
                save_to_name=os.path.join(f'{self.save_to["random_forest"]}', f'{bravais_lattice}_reg.png'),
            ...
            calibrate_regression(
                ...
                unit_cell_length=self.data_params['unit_cell_length'],
                unit_cell_key='reindexed_unit_cell',
                save_to_name=os.path.join(f'{self.save_to["random_forest"]}', f'{bravais_lattice}_reg_calibration.png'),
                unit_cell_indices=self.data_params['unit_cell_indices'],
                )
        for split_group in self.data_params['split_groups']:
            evaluate_regression(
                ...
                unit_cell_length=self.data_params['unit_cell_length'],
                unit_cell_key='reindexed_unit_cell',
                save_to_name=os.path.join(f'{self.save_to["random_forest"]}', f'{split_group}_reg.png'),
                unit_cell_indices=self.data_params['unit_cell_indices'],
                )
            calibrate_regression(
                data=self.data[self.data['split_group'] == split_group],
                unit_cell_length=self.data_params['unit_cell_length'],
                unit_cell_key='reindexed_unit_cell',
                save_to_name=os.path.join(f'{self.save_to["random_forest"]}', f'{split_group}_reg_calibration.png'),
                unit_cell_indices=self.data_params['unit_cell_indices'],
                )
        evaluate_regression(
            data=self.data,
            unit_cell_length=self.data_params['unit_cell_length'],
            unit_cell_key='reindexed_unit_cell',
            save_to_name=os.path.join(f'{self.save_to["random_forest"]}', 'All_reg.png'),
            unit_cell_indices=self.data_params['unit_cell_indices'],
            )
        calibrate_regression(
            data=self.data,
            unit_cell_length=self.data_params['unit_cell_length'],
            unit_cell_key='reindexed_unit_cell',
            save_to_name=os.path.join(f'{self.save_to["random_forest"]}', 'All_reg_calibration.png'),
            unit_cell_indices=self.data_params['unit_cell_indices'],
            )
