...
def active_merits(groups):
    ...
    merits = []
    if 'raw' in groups or 'scaled' in groups:
...
def load_scalers(models_dir, groups=DEFAULT_GROUPS, methods=SCALER_METHODS):
    """S07's normalisers, restricted to the merits this feature set actually uses.

    `FomNull.load_scalers` returns all forty-four, including `M_nn` and `chi2_fixed`, which this
    module excludes -- and a scaler whose merit column was never read raises on apply rather than
    being ignored. Filtering here keeps the exclusion stated in one place.
    """
    return FomNull.load_scalers(models_dir, merits=active_merits(groups), methods=methods)


def scaled_names(scalers, merits):
    """The `{merit}__{label}` columns `FomNull.scaled_features` will append, for these merits."""
    keep = set(merits)
    return tuple(f'{null.merit}__{null.label}' for null in scalers if null.merit in keep)
...
def feature_specification(groups=DEFAULT_GROUPS, scalers=()):
    """(names, categorical_names) for one choice of feature groups.

    The ablation in the results document is a group-drop, so the group is the unit here rather
    than the column: dropping `scaled` has to remove all thirty-odd of S07's outputs at once or
    ...
    return tuple(ordered), tuple(categorical)
...
def combiner_frames(benchmark_dir, feature_dir, bundles, keep_entry_ids, covariates, scalers,
    ...
    for bundle in bundles:
        ...
        usable = [null for null in scalers if null.merit in pool.columns]
        if usable:
            pool = FomNull.scaled_features(pool, usable)
...
class FomCombiner:
    ...
    def fit(cls, frames, groups=DEFAULT_GROUPS, scalers=(), objective='pointwise', seed=12345,
        ...
        names, categorical = feature_specification(groups, scalers)
