...
def print_results(candidates, params, file_name):
  '''
  ...
  for i in i_first_matching_unique:
    ...
    results.append(best)
  results.sort(key=lambda r: 1/r.score)
  with open(file_name, 'w') as f:
    n = min(100, len(results))
    for r in results[:n]:
      f.write("{:.4f}\t{}".format(r.score, r) + "\n")
...
class Script(object):
  ...
  def run(self):
    ...
    print_results(
      candidates_other_flat,
      params,
      params.input.peak_list.replace('.dat', '.log'),
    )
    print("Triclinic results:")
    print_results(
      candidates_triclinic_flat,
      params,
      params.input.peak_list.replace('.dat', '_triclinic.log'),
    )
