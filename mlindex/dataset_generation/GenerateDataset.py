...
from mlindex.dataset_generation.EntryHelpers import save_identifiers
...
class EntryGenerator:
    def __init__(self, lattice_system):
        ...
        self.data_set_components = {
            ...
            'reindexed_extinction_group': 'string',
    ...
    def get_peak_list(self, data):
        def pick_peaks(I_pattern, q2_pattern, q2_peaks, hkl, peak_length):
            I_pattern /= np.trapezoid(I_pattern, q2_pattern)
...
def generate_dataset(bravais_lattice, train_fraction, entries_per_group, seed=12345):
    ...
    if rank == 0:
        if not os.path.exists(os.path.join('..', 'data', 'generated_datasets')):
            os.mkdir(os.path.join('..', 'data', 'generated_datasets'))
        if os.path.exists(os.path.join('..', 'data', 'generated_datasets', 'tmp')):
            shutil.rmtree(os.path.join('..', 'data', 'generated_datasets', 'tmp'))
        os.mkdir(os.path.join('..', 'data', 'generated_datasets', 'tmp'))
    ...
    if rank == 0:
        chunks = []
        for file_name in os.listdir(os.path.join('..', 'data', 'generated_datasets', 'tmp')):
            if file_name.startswith('chunk'):
                chunks.append(pd.read_parquet(os.path.join('..', 'data', 'generated_datasets', 'tmp', file_name)))
