...
Three different representations of the unit cell are included, the conventional unit cell is taken directly from the the database. Next is a reindexed unit cell. For orthorhombic, monoclinic, and triclinic, the conventional unit cell is reindexed so the unit cell lengths, $a,\ b,\ c$ are in increasing lengths. This reindex changes the Hermann–Mauguin spacegroup symbol, unit cell parameters, and Miller indices. Rhombohedral entries that are listed in the hexagonal setting are reindexed to the rhombohedral setting. This reindex changes the unit cell parameters, Miller indices and reduces the the unit cell volume by a factor of 3. Last is the Niggli reduced unit cell, which results in changes to the unit cell parameters and volume.
